...
# load the data
# Choose one of the catalogues: M>2.5 / M>3 / M>3.5
# I have not changed the name of the region so everywhere you see Colfiorito herein, it is in fact the Amatrice! :)
my_data <- read.csv ("data/M3.5.csv" , header=TRUE)

# ds file
colfiorito.ds.97_99 <- data.frame(my_data)
colfiorito.ds.97_99[['time_date']] <- as.POSIXct(colfiorito.ds.97_99[['time_date']], format = "%d/%m/%Y %H:%M")

# sp file
colfiorito.sp.97_99 <- data.frame(my_data)
sp::coordinates(colfiorito.sp.97_99) <- c("Lon", "Lat")
colfiorito.sp.97_99[['time_date']] <- as.POSIXct(colfiorito.sp.97_99[['time_date']], format = "%d/%m/%Y %H:%M")
...
col.fit_sigma <- ETAS.fit.B_bkg(sample.s = df.bru, # data.frame representing data points 
                              ...
                              M0 = 3.5, # magnitude of completeness
...
multiplot(pl.seq + labs(title = 'Amatrice'), pl.seq2, cols = 2)
```
...
fore.week <- function(fit_, total.data, weeks.data, week.idx,
                      ...
                      beta.p = beta.p_,
                      n.samp = 1000, crs_ = italy.crs, ncore = 1){
  ...
  for(i in 1:ncol(sample.param)){
    ...
    ss <- sample.ETAS(th.p = th_, 
                      ...
                      bdy = bdy_, Sigma = Sigma_, Ht = past.week.data, ncore = ncore)
...
for(ww in 1:length(weeks.to.fore)){
  ...
  ll <- fore.week(fit_ = col.fit_sigma, total.data = df.bru, weeks.data = weeks.to.fore,
                  week.idx = ww, n.samp = 500, ncore = my_cores)
  # store them 
